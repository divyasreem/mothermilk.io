...
class Module
{
	...
	public function onBootstrap(\Zend\EventManager\EventInterface $e) // use it to attach event listeners
	{
		...
		if ($auth->hasIdentity()) {
	        $container =$this->sm->get('navigation');
	        $loginPage = $container->findBy('route' , 'auth-doctrine/default');
	     	$container->removePage($loginPage);
	     	$registration = $container->findBy('route' , 'registration');
	     	$container->removePage($registration);
